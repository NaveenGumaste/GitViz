...
import { readRepoCache, writeRepoCache } from "@/lib/repoCache";
...
async function parseRouteResponse<T>(
	response: Response,
	schema: z.ZodType<T>,
): Promise<T> {
...
async function fetchRouteData<T>(
	url: string,
	schema: z.ZodType<T>,
): Promise<T> {
...
export const useRepoStore = create<RepoStoreState>((set, get) => ({
	...
	refresh: async () => {
		...
		await get().fetchAll(current.owner, current.repo, { forceRefresh: true });
	},
	fetchAll: async (owner, repo, options) => {
		const forceRefresh = options?.forceRefresh ?? false;

		if (!forceRefresh) {
			const cachedData = readRepoCache(owner, repo);
			if (cachedData) {
				const normalizedCachedData = cachedData.commits
					? {
							...cachedData,
							commitCountsByDate: buildCommitCounts(cachedData.commits),
						}
					: cachedData;
		...
		try {
			...
			const repoMeta = await fetchRouteData(
				`/api/github/repo?${baseParams.toString()}`,
				RepoMetaSchema,
			);
			...
			const [treeResult, commitsResult, contributorsResult, languagesResult] =
				await Promise.allSettled([
					fetchRouteData(
						`/api/github/tree?${treeParams.toString()}`,
						TreeNodeSchema.array(),
					),
					fetchRouteData(
						`/api/github/commits?${baseParams.toString()}`,
						CommitSchema.array(),
					),
					fetchRouteData(
						`/api/github/contributors?${baseParams.toString()}`,
						ContributorSchema.array(),
					),
					fetchRouteData(
						`/api/github/languages?${baseParams.toString()}`,
						LanguagesSchema,
					),
				]);
			...
			if (!hasErrors) {
				writeRepoCache(owner, repo, nextData);
			}
		} catch (error) {
